...
public class CarsCont extends Attributes {
    ...
    @GetMapping("/rent/{id}")
    public String CarRent(@PathVariable Long id) {
        Cars car = carsRepo.getReferenceById(id);
    ...
    @PostMapping("/add")
    public String CarAddNew(Model model, @RequestParam String name, @RequestParam MultipartFile[] photos, @RequestParam int average, @RequestParam BodyType bodyType, @RequestParam String description, @RequestParam Fuel fuel, @RequestParam Transmission transmission, @RequestParam int capacity) {
        if (photos != null && !Objects.requireNonNull(photos[0].getOriginalFilename()).isEmpty()) {
            try {
                ...
                Cars car = new Cars(name,  true, result_photos);
    ...
    @PostMapping("/edit/{id}")
    public String CarEditOld(Model model, @RequestParam String name, @RequestParam MultipartFile[] photos, @RequestParam int average, @RequestParam BodyType bodyType, @RequestParam String description, @RequestParam Fuel fuel, @RequestParam Transmission transmission, @RequestParam int capacity, @PathVariable Long id) {
        Cars car = carsRepo.getReferenceById(id);
        String[] result_photos;
        if (photos != null && !Objects.requireNonNull(photos[0].getOriginalFilename()).isEmpty()) {
            try {
